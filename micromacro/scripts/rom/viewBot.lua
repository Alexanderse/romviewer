...
function main()
	...
	logMessage(sprintf("Using static char address 0x%X, player address 0x%X",
		tonumber(addresses.staticbase_char), tonumber(playerAddress)));
			...
			printf(tostring(functionTargetPatchAddr))
		
		
	player = CPlayer.new();
	setWindowName(getWin(), "ROM - "..player.Name);
	...
	setWindowName(getHwnd(), sprintf("RB [%s] v%s", displayname, BOT_VERSION));
	...
	while(true) do
		...
		if( player.TargetPtr ~= 0 and not player:haveTarget() and (not settings.profile.options.STOP_TARGET_CLEARING)) then
			player:clearTarget();
		end
		...
		-- don't fight, move to wp
			...
			if (__WPL:getMode() ~= "wander") then 
				success, reason = player:moveTo(wp,nil,true);
			else
			...
			if( success ) then
				...
				if (__WPL:getMode() ~= "wander") then
					...
					--sendChatMessage("Navigation", "Player failed to move to waypoint (x,z - "..wp.X..","..wp.Z..") - "..reason);
					...
					if( reason == WF_STUCK or distBreakCount > 3 ) then
						--sendChatMessage("Navigation", "Player is stuck at (x,z - "..player.X..","..player.Z..") moving to ("..wp.X..","..wp.Z..")");
