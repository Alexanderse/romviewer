...
namespace RomViewer.Model
{
    public static class World
    {
        ...
        public static List<GameNode> FindAllReferences(GameNode target)
        {
            List<GameNode> refererrers = new List<GameNode>();

            foreach (GameNode node in Data.Nodes)
            {
                List<GameNodeLink> links = node.GameNodeLinks.FindAll(link => link.Target == target);
                if (links.Count > 0) refererrers.Add(node);
            }
            ...
            if (UseTransporters)
            {
                foreach (Waypoint waypoint in Waypoints)
                {
                    foreach (GameObject go in waypoint.Node.GameObjects)
                    {
                        ...
                        {
                            waypoint.AddLink(Waypoints.Find(wp => wp.Node == link.Destination), LinkStyle.Transport,
                                             link.Script);
                        }

                        foreach (GameObjectType objectType in go.ObjectTypes)
                        {
                            if (!waypoint.SupportsObjectType.Contains(objectType))
                                waypoint.SupportsObjectType.Add(objectType);
                        }
                    }
                }
            }
