            ...
            try
            {
                foreach (Zone zone in _zones)
                {
                    ...
                    foreach (GameNode node in zone.Waypoints)
                    {
                        ...
                        //draw name
                        string caption = "";
                        if (node.Name == node.Id.ToString()) caption = node.Name;
                        else caption = string.Format("{0} [{1}]", node.Name, node.Id);

                        g.DrawString(caption, _mapSettings.NodeFont, new SolidBrush(_mapSettings.NodeTextColor), nodeRect.Right + 2, nodeRect.Top);
