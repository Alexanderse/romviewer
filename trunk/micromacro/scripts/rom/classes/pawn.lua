...
function CPawn:update()
	self:bugLog("CPawn: in CPawn:update");
	local proc = getProc();
	local memerrmsg = "Failed to read memory";
	local tmp;

	self:bugLog("CPawn: tmp = memoryReadRepeat(\"uint\", proc, self.Address + addresses.pawnId_offset) or 0;");
	tmp = memoryReadRepeat("uint", proc, self.Address + addresses.pawnId_offset) or 0;
	self:bugLog("tmp="..tostring(tmp));
	self:bugLog("self.Id="..tostring(self.Id));
	...
	self:bugLog("CPawn: tmp = memoryReadRepeat(\"byte\", proc, self.Address + addresses.charAlive_offset);");
	tmp = memoryReadRepeat("byte", proc, self.Address + addresses.charAlive_offset);
	self.Alive = not(tmp == 9 or tmp == 8);
	self:bugLog("CPawn: self.HP = memoryReadRepeat(\"int\", proc, self.Address + addresses.pawnHP_offset) or self.HP;");
	self.HP = memoryReadRepeat("int", proc, self.Address + addresses.pawnHP_offset) or self.HP;

	self:bugLog("CPawn: self.MaxHP = memoryReadRepeat(\"int\", proc, self.Address + addresses.pawnMaxHP_offset) or self.MaxHP;");
	self.MaxHP = memoryReadRepeat("int", proc, self.Address + addresses.pawnMaxHP_offset) or self.MaxHP;
	self:bugLog("CPawn: self.MP = memoryReadRepeat(\"int\", proc, self.Address + addresses.pawnMP_offset) or self.MP;");
	self.MP = memoryReadRepeat("int", proc, self.Address + addresses.pawnMP_offset) or self.MP;
	self:bugLog("CPawn: self.MaxMP = memoryReadRepeat(\"int\", proc, self.Address + addresses.pawnMaxMP_offset) or self.MaxMP;");
	self.MaxMP = memoryReadRepeat("int", proc, self.Address + addresses.pawnMaxMP_offset) or self.MaxMP;
	self:bugLog("CPawn: self.MP2 = memoryReadRepeat(\"int\", proc, self.Address + addresses.pawnMP2_offset) or self.MP2;");
	self.MP2 = memoryReadRepeat("int", proc, self.Address + addresses.pawnMP2_offset) or self.MP2;
	self:bugLog("CPawn: self.MaxMP2 = memoryReadRepeat(\"int\", proc, self.Address + addresses.pawnMaxMP2_offset) or self.MaxMP2;");
	self.MaxMP2 = memoryReadRepeat("int", proc, self.Address + addresses.pawnMaxMP2_offset) or self.MaxMP2;

	self:bugLog("CPawn: self.Race = memoryReadRepeat(\"int\", proc, self.Address + addresses.pawnRace_offset) or self.Race;");
	self.Race = memoryReadRepeat("int", proc, self.Address + addresses.pawnRace_offset) or self.Race;


	self:bugLog("CPawn: self.GUID = memoryReadShort(proc, self.Address + addresses.pawnGUID_offset) or self.GUID;");
	self.GUID = memoryReadShort(proc, self.Address + addresses.pawnGUID_offset) or self.GUID;
	self:bugLog("CPawn: self.Type = memoryReadRepeat(\"int\", proc, self.Address + addresses.pawnType_offset) or self.Type;");
	self.Type = memoryReadRepeat("int", proc, self.Address + addresses.pawnType_offset) or self.Type;

	self:bugLog("CPawn: self.Mounted = memoryReadRepeat(\"byte\", proc, self.Address + addresses.charPtrMounted_offset) ~= 1;");
	self.Mounted = memoryReadRepeat("byte", proc, self.Address + addresses.charPtrMounted_offset) ~= 1;
	self:bugLog("CPawn: self.Harvesting = memoryReadRepeat(\"int\", proc, self.Address + addresses.pawnHarvesting_offset) ~= 0;");
	self.Harvesting = memoryReadRepeat("int", proc, self.Address + addresses.pawnHarvesting_offset) ~= 0;
	self:bugLog("CPawn: self.Casting = (memoryReadRepeat(\"int\", proc, self.Address + addresses.pawnCasting_offset) ~= 0);");
	self.Casting = (memoryReadRepeat("int", proc, self.Address + addresses.pawnCasting_offset) ~= 0);

	self:updateBuffs()

	self:bugLog("CPawn: tmp = memoryReadRepeat(\"int\", proc, self.Address + addresses.pawnLootable_offset);");
	...
	self:bugLog("CPawn: local tmpp = memoryReadRepeat(\"int\", proc, self.Address + addresses.pawnAttackable_offset) or 0;");
	...
	showWarnings(false);
	self:bugLog("CPawn: local namePtr = memoryReadRepeat(\"uint\", proc, self.Address + addresses.pawnName_offset);");
...
--		cprintf(cli.yellow, "DEBUG utf8 %s %d\n", self.Name, deltaTime(getTime(), hf_before) );
	...
	self:bugLog("CPawn: self.Level = memoryReadRepeat(\"int\", proc, self.Address + addresses.pawnLevel_offset) or self.Level;");
	self.Level = memoryReadRepeat("int", proc, self.Address + addresses.pawnLevel_offset) or self.Level;
	self:bugLog("CPawn: self.Level2 = memoryReadRepeat(\"int\", proc, self.Address + addresses.pawnLevel2_offset) or self.Level2;");
	self.Level2 = memoryReadRepeat("int", proc, self.Address + addresses.pawnLevel2_offset) or self.Level2;

	self:updateTargetPtr()

	self:bugLog("CPawn: self.X = memoryReadRepeat(\"float\", proc, self.Address + addresses.pawnX_offset) or self.X;");
	self.X = memoryReadRepeat("float", proc, self.Address + addresses.pawnX_offset) or self.X;
	self:bugLog("CPawn: self.Y = memoryReadRepeat(\"float\", proc, self.Address + addresses.pawnY_offset) or self.Y;");
	self.Y = memoryReadRepeat("float", proc, self.Address + addresses.pawnY_offset) or self.Y;
	self:bugLog("CPawn: self.Z = memoryReadRepeat(\"float\", proc, self.Address + addresses.pawnZ_offset) or self.Z;");
	self.Z = memoryReadRepeat("float", proc, self.Address + addresses.pawnZ_offset) or self.Z;

	self:bugLog("CPawn: local Vec1 = memoryReadRepeat(\"float\", getProc(), self.Address + addresses.pawnDirXUVec_offset);");
	local Vec1 = memoryReadRepeat("float", getProc(), self.Address + addresses.pawnDirXUVec_offset);
	self:bugLog("CPawn: local Vec2 = memoryReadRepeat(\"float\", getProc(), self.Address + addresses.pawnDirZUVec_offset);");
	local Vec2 = memoryReadRepeat("float", getProc(), self.Address + addresses.pawnDirZUVec_offset);
	self:bugLog("CPawn: local Vec3 = memoryReadRepeat(\"float\", getProc(), self.Address + addresses.pawnDirYUVec_offset);");
	...
	self:bugLog("CPawn: local attackableFlag = memoryReadRepeat(\"int\", \proc, self.Address + addresses.pawnAttackable_offset) or 0;");
	...
	self:bugLog("CPawn: self.Class1 = memoryReadRepeat(\"int\", proc, self.Address + addresses.pawnClass1_offset) or self.Class1;");
	self.Class1 = memoryReadRepeat("int", proc, self.Address + addresses.pawnClass1_offset) or self.Class1;
	self:bugLog("CPawn: self.Class2 = memoryReadRepeat(\"int\", proc, self.Address + addresses.pawnClass2_offset) or self.Class2;");
	self.Class2 = memoryReadRepeat("int", proc, self.Address + addresses.pawnClass2_offset) or self.Class2;

	self:bugLog("CPawn: self.Speed = memoryReadRepeat(\"float\", proc, self.Address + addresses.pawnSpeed_offset)");
	self.Speed = memoryReadRepeat("float", proc, self.Address + addresses.pawnSpeed_offset)

	self:bugLog("CPawn: tmp = memoryReadRepeat(\"byteptr\",proc, self.Address + addresses.pawnSwim_offset1, addresses.pawnSwim_offset2)");
...
DEBUG_PlayerRefreshBug=false;
function CPawn:bugLog(_text)
	if (DEBUG_PlayerRefreshBug == true) then printf(tostring(_text).."\n"); end;
end;
